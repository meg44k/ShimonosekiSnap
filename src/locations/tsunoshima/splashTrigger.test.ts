...
// getWhaleTransform()のY座標は t≈0.1082(elapsedMs≈595ms)で離水方向に、
// t≈0.8918(elapsedMs≈4905ms)で着水方向にSEA_LEVEL_Y(-0.07)を横切る
// (WAYPOINTSから導いたベジェ曲線 y(t) = -0.15 + 0.829t(1-t) を -0.07 で解いた値。
...
describe('detectSplashCrossing', () => {
  ...
  it('returns the crossing-frame position for use as the splash spawn point', () => {
    const event = detectSplashCrossing(580, 620)
    expect(event).not.toBeNull()
    expect(event?.position[0]).toBeCloseTo(-0.1956, 3)
    expect(event?.position[1]).toBeCloseTo(-0.0671, 3)
    expect(event?.position[2]).toBeCloseTo(0.016, 3)
  })
